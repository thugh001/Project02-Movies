...
def about():
    ...
    gm=googlemaps.Client(key=api_key)
    addlist=[]
    for el in data_dic['data']['movies'][0]['filmingLocations']:
        addlist.append(gm.geocode(el['location']))
    for a in addlist:
        for b in a:
            latlong.append((b['geometry']['location']))
    genres=data_dic['data']['movies'][0]['genres']
    urlPoster=data_dic['data']['movies'][0]['urlPoster']
    parameters=[title,address,releaseDate,year,latlong,genres,urlPoster]
